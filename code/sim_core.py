    ...
    for step in range(steps):
        ...
        if step % 250 == 0 and growth_by_division and born_n < N:
            ...
            if growth_from_coincidence:
                # СОВПАДЕНИЕ, а не активность. Читается не величина
                # сдвига весов, а его СОСРЕДОТОЧЕННОСТЬ: повторяющийся
                # образ двигает НЕМНОГИЕ связи СИЛЬНО, бессвязная
                # активность -- многие понемногу. Измерено на 60 тканях
                # парно: сосредоточенность сдвига 0.442 против 0.379
                # (+16.6%, 49 из 60, p = 3.8e-07), тогда как СУММА сдвига
                # не различает ничего (+1.3%, 29 из 60).
                d_w = np.abs(W - 0.015) * contacts
                s1 = d_w.sum(axis=1)
                s2 = (d_w * d_w).sum(axis=1)
                dev = np.zeros(N)
                nz = s1 > 1e-12
                dev[nz] = s2[nz] / s1[nz] ** 2
                # Мера БЕЗУСЛОВНАЯ, а не относительно своей же ткани.
                # Сосредоточенность умножается на число связей: 1 -- сдвиг
                # размазан поровну, deg -- собран на одной связи. Первая
                # редакция делила на среднее ПО ЖИВЫМ УЗЛАМ той же ткани и
                # тем самым вычитала ровно то, что хотела увидеть:
                # различие строения -- МЕЖДУ тканями, а не между узлами
                # внутри одной, и нормировка на собственное среднее его
                # сокращала начисто (38.8 узла против 38.3 при разнице
                # исходной величины в 16.6%).
                deg_w = np.maximum(contacts.sum(axis=1), 1)
                spread = np.maximum(deg_w - 1, 1)
                boost = np.clip((dev * deg_w - 1.0) / spread, 0.0, 1.0)
