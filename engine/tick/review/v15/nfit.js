...
const RULE   = K('RULE', 0);          // 1 закон учёбы в генах, 0 LMS рукой. По умолчанию рукой:
                                      // на 24 сидах генный закон давал 1.88 бита против 2.89 и 4 вымирания
                                      // из 24 против 0. Почему отбор не находит LMS -- отдельный вопрос
...
function newPart(w, slot, par) {
  ...
  p.kids = 0; p.born = w.round; p.sb = 0; p.sg = 0; p.sl = 0; p.ns = 0; p.paid = 0;
  p.d = RULE ? Math.abs(Math.log(g.b / g.a)) + g.c / g.a : 0;
  if (par) par.kids = (par.kids || 0) + 1;
...
function kill(w, p, why) {
  ...
  if (w.log && p.born > 10000) w.log.push({ d: p.d, kids: p.kids, life: p.age, bits: p.ns ? p.sb / p.ns : 0, gain: p.ns ? p.sg / p.ns : 0, links: p.ns ? p.sl / p.ns : 0, paid: p.paid / Math.max(1, p.age), got: p.fromReads / Math.max(1, p.age), world: p.fromWorld / Math.max(1, p.age) });
...
function round(w) {
  ...
  for (const p of P) {
    ...
    for (const l of order) {
      if (!(FREE_TRY > 0 && l.age < TRIAL)) {   // проба -- бесплатный образец, остальное за плату
        ...
        p.credit -= PRICE; inc[l.j] += PRICE; p.paid += PRICE;
      }
  ...
  for (const p of P) {
    ...
    p.sb += p.bits; p.sg += p.gain; p.sl += p.links.length; p.ns++;
    p.age++;
  }
  ...
  for (const s of free) {
    ...
    if (w.blog) { const m = pool.reduce((x, y) => x + y.gain, 0) / pool.length,
      mx = pool.reduce((x, y) => Math.max(x, y.gain), -1e9);
      w.blog.push({ n: pool.length, spread: m > 0 ? mx / m : 0 }); }
