...
def simulate(
    ...
    Строение различает только то, что опирается на СОВПАДЕНИЕ: у
    пластичности веса разъезжаются вдвое сильнее, когда образ повторяется
    (max|dW| 1.54e-3 против 6.94e-4 при равном числе спайков). Здесь этот
    же признак отдан росту.
    ...
    for step in range(steps):
        ...
        if step % 250 == 0 and growth_by_division and born_n < N:
            ...
            if growth_from_coincidence:
                # СОВПАДЕНИЕ, а не активность: насколько веса узла УЕХАЛИ
                # от исходных. Повторяющийся образ двигает их вдвое
                # сильнее, чем такая же по силе, но бессвязная активность
                # (измерено: max|dW| 1.54e-3 против 6.94e-4).
                deg_w = np.maximum(contacts.sum(axis=1), 1)
                dev = (np.abs(W - 0.015) * contacts).sum(axis=1) / deg_w
                m = dev[live].mean()
                if m > 0:
                    # ТОЛЬКО УСКОРЯЕТ, никогда не тормозит. Первая редакция
                    # брала (dev/m - 1) со знаком, и при силе 1 и выше рост
                    # вставал намертво: у большинства узлов совпадения нет,
                    # множитель уходил в нуль и глушил деление вовсе (12
                    # тканей из 12 не дорастали). Прибавка, способная стать
                    # вычитанием, -- не та же ручка.
                    boost = np.maximum(dev[live] / m - 1.0, 0.0)
                    chance = chance * (1.0 + growth_from_coincidence * boost)
