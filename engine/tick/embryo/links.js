// Замер PRE49_LINKS.md: причина исчезновения каждой проверенной связи за 100 кругов свободной активности.
...
console.log([seed, T.length, T.filter((t) => t.cause).length, cnt(T).join(','), R.length, R.filter((t) => t.cause).length, cnt(R).join(','), byK5.join(','), heldSig].join('\t'));
