...
if (process.argv[2] === 'П2') {
  ...
  const M = require('./neuron2.js'), seed = +process.argv[2], mode = M.CFG.HOLD ? 'H' : '0', w = M.create(seed);
  ...
  console.log(`П1 сид ${seed} HOLD=${M.CFG.HOLD}: частей ${A.parts.length}, расхождение прогнозов ${dp.toExponential(1)}, сигналов ${dz.toExponential(1)} (${nz} без ограничения) -> ${dp <= 1e-9 && dz <= 1e-9 ? 'выполнена' : 'НЕ выполнена'}`);
}
