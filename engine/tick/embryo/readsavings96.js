...
const fs = require('fs'), TM = +(process.argv[3] ?? 1);   // шаг 96: правило второй проверки (1 -- «−», как было)
...
function sav(a, b) {
  ...
  for (const s of seeds) {
    const ap = E(a, s, 0), am = E(a, s, TM), bp = E(b, s, 0), bm = E(b, s, TM);
...
for (const [lab, name] of [['A', 'прошлое +, промежуток 2000'], ['B', `прошлое ${TM === 1 ? '−' : TM}, промежуток 2000`], ['A0', 'прошлое +, без промежутка'], ['B0', `прошлое ${TM === 1 ? '−' : TM}, без промежутка`], ['Bn', 'прошлое + (другой шум), промежуток 2000']])
  console.log(`  ${name.padEnd(40)} проверка +: ${f(med(seeds.map((s) => E(lab, s, 0))))}, проверка ${TM === 1 ? "−" : TM}: ${f(med(seeds.map((s) => E(lab, s, TM))))}`);
