"""v0.50: признак успеха ПОСВЯЗНО -- рядом с хеббовской пластичностью.

ОТКУДА. v0.49 измерил построчный признак успеха и нашёл его немым: ни
пользы (-0.09 п.п.), ни предсказанной скуки. Диагноз был не догадкой:
построчный множитель умножает всю строку весов на одно число, то есть
меняет, НАСКОЛЬКО узел учится, но не КАКИЕ связи усиливаются. А в проекте
трижды измерено, что решает рисунок, а не величина (v0.29, v0.41, v0.44).

ЧТО ИЗМЕНЕНО. Признак действует ПОСВЯЗНО:
    W[i,j] -= сила * (syn_i - pred_i) * trace_j
Связь, деятельная перед входом СВЕРХ ожидания, слабеет; перед входом
МЕНЬШЕ ожидаемого -- крепнет. Ткань делает свой вход предсказуемым по
каждой связи отдельно.

ХЕББОВСКАЯ ПЛАСТИЧНОСТЬ СОХРАНЕНА НЕТРОНУТОЙ. Это уточнение автора
(PRINCIPLES §2b): "сохрани ту же пластичность, ведь в биологии это
главное, что умеет мозг". Добавка стоит РЯДОМ с прежним правилом, а не
вместо него, и при нулевой силе тождество побитово (проверено, 9 из 9).

КАЛИБРОВКА показала, что добавка действует сильно и что у неё есть
вырожденный предел:

    сила | сдвиг весов от исходных | спайков | двиг. | разброс мира
       0 |                   0.275 |     444 |  46.6 |       0.1430
    1e-4 |                   1.060 |     208 |  19.5 |       0.1359
    5e-4 |                   0.891 |      87 |   2.0 |       0.1376
    2e-3 |                   0.658 |      65 |   0.0 |       0.1386

Веса при 1e-4 движутся ВЧЕТВЕРО активнее -- пластичность не подавлена, а
усилена. Но ткань стихает, а при большой силе замолкает вовсе: самый
простой способ сделать свой вход предсказуемым -- обнулить его. Взята
рабочая точка 1e-4, где ткань ещё жива.

СКУКА ЗДЕСЬ -- НЕ ИЗЪЯН. По уточнению автора стремление к предвидимому
совместимо с сохранённой пластичностью и записывается описательно, а не
как цена: "наш мозг тоже так делает, ему не нравится что-то новое, он
привык к привычному, но тем не менее пластичность имеется".
...
def run(seed, gain):
    return simulate(seed=seed, stimulus=[v46.touch], stimulus_amp=0.4,
                    ...
                    activity_memory=0.05, predict_per_link=gain, **BASE)
...
def main():
    ...
    if mism > 0.10 or not (0.42 <= nul_all <= 0.58):
        ...
        print(f"  разница меньше {MIN_EFFECT * 100:.0f} п.п. -- "
              "посвязный признак на читаемость НЕ ВЛИЯЕТ")
    elif p < 0.05:
        print("  ПОСВЯЗНЫЙ ПРИЗНАК УСПЕХА ПОМОГАЕТ")
    else:
