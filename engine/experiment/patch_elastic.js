...
'use strict';
/* ============================================================
   ЭКСПЕРИМЕНТ (версия с упругой тканью и усиленным контролем).

   Отличия от patch.js:
   1. принимает параметры закреплённых контактов -- проверка на
      материале, ставшем упругим (этап 11);
   2. КОНТРОЛЬ ПОЛОЖЕНИЯ усилен: вместо аналитического ожидания
      (360/(4*число границ)) строится ТОЧНОЕ нулевое распределение
      исчерпывающим ПОВОРОТОМ. Данные круговые (36 секторов), поэтому
      перебираются все 36 сдвигов участка относительно кривизны:
      форма участка и профиль кривизны сохраняются полностью, рушится
      только их взаимная привязка. Аналитическое ожидание предполагает
      равномерность секторов и равновероятность положения максимума;
      поворотный нуль этих допущений не требует и потому сильнее.
   3. пишет в отдельный файл -- не смешивать с данными этапа 10.
...
const JADH = +(process.argv[4] || 0);
const JST = +(process.argv[5] || 0);
...
function analyse(w, gene) {
  ...
  /* ТОЧНЫЙ ПОВОРОТНЫЙ НУЛЬ. Участок сдвигается по кругу на все 36
     позиций; профиль кривизны и форма участка сохраняются, рушится
     лишь их взаимная привязка. Наблюдение -- сдвиг 0. */
  let rotNull = null, rotP = null, rotMean = null;
  if (edges.length) {
    const dists = [];
    for (let sh = 0; sh < NB; sh++) {
      const Esh = E.map((_, i) => E[(i - sh + NB) % NB]);
      const ed = [];
      for (let i = 0; i < NB; i++) {
        const a = Esh[i], b = Esh[(i + 1) % NB];
        if ((a - 0.5) * (b - 0.5) < 0) ed.push(i);
      }
      if (!ed.length) continue;
      let d2 = NB;
      for (const e of ed) d2 = Math.min(d2, Math.min(Math.abs(bi - e), NB - Math.abs(bi - e)));
      dists.push(d2 * STEP);
    }
    if (dists.length) {
      rotNull = dists;
      rotMean = dists.reduce((a, b) => a + b, 0) / dists.length;
      // доля поворотов, дающих расстояние НЕ БОЛЬШЕ наблюдённого
      rotP = dists.filter((d2) => d2 <= distToEdge).length / dists.length;
    }
  }
  return { by, byK, edges: edges.length, distToEdge, expected, rotP, rotMean, K, E, D };
}

const OUT = path.join(__dirname, '..', 'results', 'patch_elastic.mine.jsonl');
...
for (const seed of SEEDS) {
  ...
  const rec = {
    ...
    dist: a.distToEdge, expected: a.expected, rotP: a.rotP, rotMean: a.rotMean,
  };
  rows.push(rec);
  ...
  console.log(`сид ${String(seed).padStart(4)} | границ участка ${rec.edges} | деформация внутри ${(100 * rec.dIn).toFixed(0)}% ` +
    ...
    `до границы ${rec.dist === null ? '—' : rec.dist.toFixed(0) + '°'} | аналит.ожид ${rec.expected === null ? '—' : rec.expected.toFixed(0) + '°'} | поворотный нуль ${rec.rotMean === null ? '—' : rec.rotMean.toFixed(0) + '°'} p=${rec.rotP === null ? '—' : rec.rotP.toFixed(2)}`);
}
...
if (ok.length) {
  ...
  const withRot = ok.filter((r) => r.rotP !== null && r.rotMean !== null);
  if (withRot.length) {
    const rm = withRot.reduce((s2, r) => s2 + r.rotMean, 0) / withRot.length;
    const rp = withRot.reduce((s2, r) => s2 + r.rotP, 0) / withRot.length;
    const sig = withRot.filter((r) => r.rotP <= 0.05).length;
    console.log(`  ПОВОРОТНЫЙ НУЛЬ (точный, 36 сдвигов): среднее ${rm.toFixed(0)}° против аналитического ${e.toFixed(0)}°`);
    console.log(`    доля поворотов не дальше наблюдённого: среднее p=${rp.toFixed(3)}; p<=0.05 у ${sig} из ${withRot.length} сидов`);
    console.log(`    (привязка к границе означала бы МАЛОЕ p; p около 0.5 -- положение случайно)`);
  }
}
