...
function create(seed) {
  const w = { rnd: makeRNG(seed), round: 0, parts: [], c: new Float64Array(CH), births: 0,
    ...
    fs: { acts: 0, hits: 0, rounds: 0, fed: 0, winners: 0, burn: 0, sf: 0, n: 0,
          ...
          byK: new Map(), dUse: new Array(M_PL).fill(0), dHit: new Array(M_PL).fill(0), pert: 0, pertCh: 0 } };  // сколько связей каждого товара отмерло и сколько потеряно со смертью продавца
...
function round(w) {
  ...
  if (EAT && w.fq) {                      // действие: добытчик платит ACT и называет долю
    ...
    for (const p of P) {
      ...
      let eps = 0, pv = p.pred;
      if (GAINM) { eps = SIG > 0 ? SIG * gauss(w.rnd) : 0; pv = (p.mul + eps) * p.pred;
      ...
      w.acts.push({ slot: p.slot, place, d, eps });
      sum += mPlace(place); k++;
    }
