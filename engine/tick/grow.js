...
/* Тактов на круг, ФИКСИРОВАН. Был вписан числом; теперь задаётся, чтобы
   мир можно было делать больше при ТОМ ЖЕ достатке части. Шаг 20 нашёл,
   зачем: при 64 частях у каждой около двадцати соседей -- треть мира, и
   округи не существует вовсе. Умолчание прежнее, тождество цело. */
const BUDGET = num('BUDGET', 200);
...
/* ============ У АДРЕСА ЕСТЬ ПОРЯДОК ============

   Шаг 20: при 64 частях у каждой около двадцати соседей -- треть мира.
   Мир сделан больше, и прямых соседей стало 11% вместо 35%, НО за два
   шага по-прежнему покрывается 89-99%. Причина названа: встречные
   берутся жребием, значит граф случаен, а у случайного графа округи не
   бывает ни при каком размере.

   Корень -- в самом замысле среды: «адреса без расстояния». Это и было
   её цифровым преимуществом, и оно же делает округу невозможной.

   KIN_NEAR = 1 снимает противоречие, ничего не занимая у вещества:
   расстояния по-прежнему нет, но у адреса есть ПОРЯДОК -- он и так
   число. Часть осматривает не случайных встречных, а ближайших ПО
   АДРЕСУ. Тогда у мира появляется устройство, порождённое историями
   самих частей, а не вложенной геометрией.
  ...
  for (const p of P) {
    ...
    if (p.credit >= C_LINK && w.rnd() < p.par.urge * 0.25) {
      ...
      if (KIN === 0) {
        ...
        // встречные -- БЛИЖАЙШИЕ ПО АДРЕСУ, а не случайные. Расстояния
        // нет, есть порядок: адрес -- число. Случайных чисел здесь не
        // тратится вовсе, поэтому при KIN_NEAR = 0 тождество цело.
        const mine = addrOf(w, p);
        const cand = [];
        for (let c = 0; c < P.length; c++) {
          if (c === p.id) continue;
          const d = Math.abs(addrOf(w, P[c]) - mine);
          if (cand.length < KIN_S) { cand.push({ c, d }); cand.sort((a, b) => a.d - b.d); }
          else if (d < cand[KIN_S - 1].d) { cand[KIN_S - 1] = { c, d }; cand.sort((a, b) => a.d - b.d); }
        }
        j = cand.length ? cand[KIN > 0 ? 0 : cand.length - 1].c : p.id;
      } else {
