...
/* ============================================================
   ...
   а того, чей сигнал собран из тех же входов, -- обычно его потомка.
...
function kill(w, p, why) {
  ...
  for (const q of w.parts) {
    ...
    for (const l of gone) {               // проверенная связь -- к другому продавцу того же канала
      ...
      q.links.push({ j: pick.slot, k: l.k, w: l.w, u: l.u, r2: l.r2, age: l.k === 2 ? 0 : l.age });
    }
  }
}
...
function round(w) {
  ...
  for (const p of P) {
    ...
    if (RULE) {                           // (a*s' - b*p)*x - c*w, нормировано входом
      ...
      for (let i = 0; i < p.xl.length; i++) {
        const xi = p.x[i + 1];
        p.xl[i].w += p.xt[i] ? p.g.lr * e * xi / (1 + xi * xi) : k * xi;
      }
    }
  ...
  if (SIGNAL) for (const p of P) {       // продавцы учат сигнал на ошибках покупателей
    ...
    const k = ZLR * fb[p.slot] / (n2 * Math.sqrt(p.zv));   // сигнал нормирован: градиент идёт через делитель
  ...
  for (const s of free) {
    ...
    if (!pool.length && P.filter((p) => p && p.ch === k).length < Math.ceil(N / CH)) pool = P.filter(ok);
