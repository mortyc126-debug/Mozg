...
for (const T of [10, 30]) {
  ...
  for (let c = 0; c < 20; c++) {
    ...
    for (let k = 1; k <= T; k++) { silent(); blow = Math.max(blow, rms(all(), (p) => p.pred) / a0); }
...
console.log([cond, seed, f(out[10].h0), f(out[10].hT), f(out[30].h0), f(out[30].hT), f(out[10].act), f(out[30].act), f(blow),
