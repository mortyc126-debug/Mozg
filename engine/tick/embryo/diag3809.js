...
// разбор шага 65 (после чтения): тот же путь, что у батареи; по каждой паузе T=30 -- радиус 'LC' и наибольший |прогноз| медленного канала
const LIN = require('./lin.js');
for (const T of [10, 30]) {
  const hT = acc();
  for (let c = 0; c < 100; c++) {
    for (let r = 1; r <= 100; r++) life();
    let rr = NaN; if (T === 30) rr = LIN.eig([LIN.build(w, C, 'LC')])[0].r;
    let mx = 0; for (let k = 1; k <= T; k++) { silent(); for (const p of Sp()) mx = Math.max(mx, Math.abs(p.pred)); }
    const prT = Sp().map((p) => p.pred), kpT = life(); const before = hT.net; add(hT, prT, kpT, w.L);
    if (T === 30 && (mx > 20 || rr > 1)) console.log(`T=30 цикл ${c}: радиус ${rr.toFixed(4)}, наибольший |прогноз S| в паузе ${mx.toExponential(2)}, вклад в сумму ткани ${(hT.net - before).toExponential(2)}, частей S ${Sp().length}, |wSelf+ws| S: ${Sp().map((p) => (p.wSelf + p.ws).toFixed(2)).join(' ')}`);
  }
  console.log(`T=${T}: ткань ${hT.net.toExponential(3)}, нуль ${hT.nul.toFixed(0)}, оптимум ${hT.kal.toFixed(0)}`);
}
process.exit(0);
