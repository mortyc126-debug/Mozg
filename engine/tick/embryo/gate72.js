// Ворота шага 72 (PRE72_HOLDS.md §3): путь строки 0 батареи зародыша как есть; перед каждой тишиной -- радиус 'LC' (как есть) и 'LS' (HOLDS)
...
for (const T of [10, 30]) {
  for (let c = 0; c < 100; c++) {
    ...
    const [a, b] = LIN.eig([LIN.build(w, C, 'LC'), LIN.build(w, C, 'LS', SCH)]);
    console.log([seed, T, c, a.r.toFixed(5), b.r.toFixed(5)].join('\t'));
