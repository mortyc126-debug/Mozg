...
const fs = require('fs');
const R = fs.readFileSync('out/order78.tsv', 'utf8').trim().split('\n').map((l) => l.split('\t'));
...
for (const c of ['ЮЛС', 'ЮЛС100', 'ЮЛС140', 'ЮЛ140']) {
  const A = rows(c);
...
const s = sign(rows('ЮЛС140'), rows('ЮЛС'));
console.log(`\nпоиск: ЮЛС140 выше ЮЛС по доле частей Q с линиями к A и B -- выше ${s.up}, ниже ${s.dn}, p = ${f(s.p, 4)} -> ${s.up > s.dn && s.p < 0.05 ? 'РАБОТАЕТ КАК ПОИСК' : s.dn > s.up && s.p < 0.05 ? 'УВОДИТ ОТ НУЖНОГО' : 'не установлено'}`);
const dead = R.filter((r) => !(+r[6] > 0)).map((r) => r[0] + ' ' + r[1]);
const okPos = ['ЮЛС', 'ЮЛС100', 'ЮЛС140', 'ЮЛ140'].every((c) => med(rows(c).map((r) => +r[4])) >= 0.85);
console.log(`проверки меры: предсказатель >= 0.85 во всех условиях ${okPos ? 'да' : 'НЕТ'}; вымерших ${dead.length ? dead.join(', ') : 'нет'}`);
for (const c of ['ЮЛС100', 'ЮЛС140']) { const m = med(rows(c).map((r) => +r[2])); console.log(`строка 8 в ${c}: ${f(m)} -> ${m >= 0.75 ? 'ЕСТЬ (засчитывается, если вариант принят)' : 'НЕТ'}`); }
