...
function worldStep(w) {
  ...
  if (!WORLD) { for (let k = 0; k < CH; k++) n[k] = 0.9 * c[k] + Math.sqrt(0.19) * g(); }
    ...
    if (DEEP) {                           // источник 8 и канал 9: c9(t+1) = c8(t - DEEP)
      ...
      if (EAT) { n[FCH] = (hid - LOOP * (LAG ? w.abarOld : w.abar)) / FDIV; w.hid = hid; }
...
function create(seed) {
  const w = { rnd: makeRNG(seed), round: 0, parts: [], c: new Float64Array(CH), births: 0,
    ...
    abar: 0, abarOld: 0, hid: 0, acts: [], fsamp: [], fq: null,    // среднее названное место, выборка F, границы долей
    fs: { acts: 0, hits: 0, rounds: 0, fed: 0, winners: 0, burn: 0, sf: 0, n: 0,
          ...
          byK: new Map(), dUse: new Array(M_PL).fill(0), dHit: new Array(M_PL).fill(0) } };  // сколько связей каждого товара отмерло и сколько потеряно со смертью продавца
...
function round(w) {
  ...
  if (EAT && w.fq) {                      // действие: добытчик платит ACT и называет долю
    ...
    for (const p of P) {
      ...
      const base = (DECIDE && NOPRED) ? binOf(w, 0) : binOf(w, p.pred);   // доля, в которую попал прогноз
      let d = 0;
      ...
      w.acts.push({ slot: p.slot, place, d });
      sum += mPlace(place); k++;
    }
