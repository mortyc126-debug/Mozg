...
const DEFAULTS = {
  ...
  /* полуширина неподвижного градиента. За её пределами поле упирается в
     0 или 1 и градиент там СТРОГО НУЛЕВОЙ. Прежде величина была вшита
     как 95 при мире шириной 1024 и пласте длиной около 390, поэтому
     ткань, сносимая полем, выходила в плоскую область и опору теряла:
     у двурядного пласта к шагу 400 в насыщении оказывались 75%
     агентов, к шагу 1400 -- 98%. Значение по умолчанию оставлено
     прежним, чтобы поведение движка не изменилось. */
  gradHalf: 95,             // полуширина градиента среды
...
function createWorld(opt = {}) {
  ...
  const ux = Math.cos(w.theta), uy = Math.sin(w.theta), half = w.p.gradHalf;
