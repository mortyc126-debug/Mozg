// Чтение шага 95, строка 6 (вторая половина): перекрёстная проба в едином мире, U1 как X, U2 как Y
const R = ['U1', 'U2'].flatMap((v) => require('fs').readFileSync('out/cross95_' + v + '.tsv', 'utf8').trim().split('\n').map((l) => l.split('\t')).map((r) => [(v === 'U1' ? 'X' : 'Y') + r[0], ...r.slice(1)])); const R0 = require('fs').readFileSync('/dev/null', 'utf8').trim().split('\n').map((l) => l.split('\t'));
