const fs = require('fs');
const K=['cond','seed','share','rate','winners','burn','varF','sf','relay','bare','nRelay','nBare','acts','aliveF','gAct','net','give','alive','bitsX','bits9','right','nul','byK','dUse','dHit','qArg','gain','pertCh','gainAll'];
const D = fs.readFileSync('out/gain.tsv','utf8').trim().split('\n').map(l=>l.split('\t'))
  .map(r=>Object.fromEntries(K.map((k,i)=>[k, i<2||i>=22&&i!=26&&i!=27?r[i]:+r[i]])));
...
for (const [n,a] of [['А настоящий мир, без решения',A],['Б настоящий мир, РЕШЕНИЕ',B],
                     ['В запаздывающий, без решения',V],['Г запаздывающий, решение',G],])
...
for (const c of ['Б','Г']) { const d=by(c);
  console.log(`  ${c}: выученные множители по сидам: ${d.map(x=>(+x.gain).toFixed(2)).join(' ')} | медиана ${med(d.map(x=>+x.gain)).toFixed(3)}`);
