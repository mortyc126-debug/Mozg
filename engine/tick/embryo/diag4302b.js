// Разбор шага 71, после чтения: внутри свободной активности цикла 2 строки 3 (PRE71_4302.md): путь батареи зародыша побитово, только чтения; по циклам строк 0 и 3 -- радиус 'LC',
...
for (let c = 0; c < 5; c++) {
  ...
  const Bm = LIN.build(w, C, 'LC'), e = LIN.eig([Bm])[0], wb = maxW();
  let mx = 0, mwho = null; for (let k = 1; k <= FF; k++) {
    const d0 = w.deaths.fault + w.deaths.bank, lk0 = A_().reduce((s, p) => s + p.links.length, 0);
    const snap = c === 2 && k === 2 ? A_().map((p) => ({ p, self: [p.wSelf, p.ws], links: p.links.map((l) => ({ l, j: l.j, k: l.k, w: l.w, age: l.age })) })) : null;
    M.freeRound(w); const [m, who] = maxPred(); if (m > mx) { mx = m; mwho = who; }
    if (snap) {                              // что изменилось в матрице за круг 2 и что из этого поднимает радиус
      const gone = [], moved = [];
      for (const s of snap) for (const o of s.links) {
        if (!s.p.links.includes(o.l)) gone.push({ s, o });
        else if (Math.abs(o.l.w - o.w) > 0.01) moved.push(`#${s.p.slot}<-#${o.j}/${o.k} w ${o.w.toFixed(3)}->${o.l.w.toFixed(3)}`);
        else if (o.age < C.TRIAL && o.l.age >= C.TRIAL) moved.push(`#${s.p.slot}<-#${o.j}/${o.k} проба окончена, w ${o.l.w.toFixed(3)}`);
      }
      for (const s of snap) { const d = Math.abs(s.p.wSelf + s.p.ws - s.self[0] - s.self[1]); if (d > 0.01) moved.push(`#${s.p.slot} себя ${(s.self[0] + s.self[1]).toFixed(3)}->${(s.p.wSelf + s.p.ws).toFixed(3)}`); }
      console.log(`  КРУГ 2: отмерло ${gone.length}: ${gone.map(({ s, o }) => `#${s.p.slot}(${GR(s.p.ch)})<-#${o.j}(${w.parts[o.j] ? GR(w.parts[o.j].ch) : '?'})/товар ${o.k} w ${o.w.toFixed(3)} u ${o.l.u.toFixed(3)} возраст ${o.age}`).join('; ')}`);
      console.log(`  КРУГ 2: изменилось заметно: ${moved.length ? moved.join('; ') : 'ничего'}`);
      const r0 = LIN.eig([LIN.build(w, C, 'LC')])[0].r;
      for (const { s, o } of gone) { s.p.links.push(o.l); const r1 = LIN.eig([LIN.build(w, C, 'LC')])[0].r; s.p.links.pop();
        console.log(`  вернуть связь #${s.p.slot}<-#${o.j}/${o.k}: радиус ${r0.toFixed(4)} -> ${r1.toFixed(4)}`); }
      const all = gone.map(({ s, o }) => { s.p.links.push(o.l); return s; }); const rA = LIN.eig([LIN.build(w, C, 'LC')])[0].r; for (const s of all) s.p.links.pop();
      console.log(`  вернуть все отмершие: радиус ${rA.toFixed(4)}`);
      const tl = w.parts[44] && w.parts[44].links.find((l) => l.j === 92 && l.k === 1);
      if (tl) { const a = tl.age; tl.age = C.TRIAL - 1; const rT = LIN.eig([LIN.build(w, C, 'LC')])[0].r; tl.age = a;
        console.log(`  оставить #44<-#92/1 в пробе: радиус ${r0.toFixed(4)} -> ${rT.toFixed(4)}`);
        const p92 = w.parts[92], p44 = w.parts[44];
        const g = (p) => (p.wSelf + p.ws).toFixed(3) + ' (в жизни на себя только ws ' + p.ws.toFixed(3) + ')';
        console.log(`  петля: #92 на себя ${g(p92)}; #44 на себя ${g(p44)}; #92<-#44 w ${(p92.links.find((l) => l.j === 44 && l.k === 1) || { w: NaN }).w.toFixed(3)}; #44<-#92 w ${tl.w.toFixed(3)}`);
        for (const p of [p92, p44]) console.log(`   #${p.slot} покупает: ${p.links.map((l) => `#${l.j}(${w.parts[l.j] ? GR(w.parts[l.j].ch) : '?'})/${l.k} w ${l.w.toFixed(3)} возраст ${l.age}`).join('; ')}`);
      }
    }
    if (c === 2 && (k <= 3 || k % 5 === 0 || w.deaths.fault + w.deaths.bank > d0)) {
      const Bk = LIN.build(w, C, 'LC'), ek = LIN.eig([Bk])[0];
      console.log(`  цикл 2 круг ${k}: |прогноз| ${m.toExponential(2)} у #${who.slot} (${GR(who.ch)}), радиус ${ek.r.toFixed(4)} (${share(ek, Bk.parts)}), смертей +${w.deaths.fault + w.deaths.bank - d0}, связей ${lk0}->${A_().reduce((s, p) => s + p.links.length, 0)}, вес ${maxW().toFixed(2)}`);
      if (k === 20) describe(who, 'часть с наибольшим прогнозом на 20-м круге');
    }
  }
