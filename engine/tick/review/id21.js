const A = require('./v20/neuron2.js'), B = require('./v21/neuron2.js');
const seed = +process.argv[2], wa = A.create(seed), wb = B.create(seed);
const f = (w) => w.parts.map(p => p ? `${p.slot}:${p.credit.toFixed(12)}:${p.pred.toFixed(12)}:${p.ate.toFixed(9)}:${p.nAct}:${p.nHit}:${p.mul.toFixed(12)}:${p.links.length}` : '-').join('|') + '#' + w.abar.toFixed(12);
