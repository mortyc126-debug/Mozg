// Ворота шага 68 (PRE68_CSEARCH.md §4): процентиль ранга лучшей линии к A и к B среди всех годных кандидатов частей канала Q
...
for (const p of P) {
  ...
  for (let j = 0; j < N; j++) for (let k = 0; k < 4; k++) {
    ...
    const s = M.cscore(w, p, j, k); sc.push(s);
    if (k === 3 && P[j].ch === O) bA = Math.max(bA, s);
    if (k === 3 && P[j].ch === O + 1) bB = Math.max(bB, s);
  }
  const pct = (x) => x < 0 ? NaN : (sc.filter((v) => v < x).length + 0.5 * sc.filter((v) => v === x).length) / sc.length;
  out.push([pct(bA), pct(bB)]);
}
const f = (x) => (Number.isFinite(x) ? x.toFixed(3) : 'NaN');
console.log([cond, seed, out.length, out.map((o) => f(o[0])).join(','), out.map((o) => f(o[1])).join(','), decode(1).toFixed(4), decode(7).toFixed(4), rep.length].join('\t'));
