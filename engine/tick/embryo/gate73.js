// Ворота шага 73 (PRE73_UNBOUNDED.md §3): путь строки 0 батареи зародыша как есть; перед каждой тишиной -- полный радиус 'LC',
// неограниченный радиус (блок прогноз -> прогноз, SIGNAL=0) в 'LC' и 'LS'; за тишину -- правда: наибольший |прогноз| ткани
...
for (const T of [10, 30]) {
  for (let c = 0; c < 100; c++) {
    ...
    const C0 = { ...C, SIGNAL: 0 };
    const [a, u, v] = LIN.eig([LIN.build(w, C, 'LC'), LIN.build(w, C0, 'LC'), LIN.build(w, C0, 'LS', SCH)]);
    let mx = 0; for (let k = 1; k <= T; k++) { silent(); for (const p of w.parts) if (p) mx = Math.max(mx, Math.abs(p.pred)); }
    console.log([seed, T, c, a.r.toFixed(5), u.r.toFixed(5), v.r.toFixed(5), mx.toExponential(3)].join('\t'));
    life();
  }
}
process.exit(0);
