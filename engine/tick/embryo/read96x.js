// Чтение шага 96, строка 6 (вторая половина): перекрёстная проба в едином мире, Z1 как X, Z2 как Y
const R = ['Z1', 'Z2'].flatMap((v) => require('fs').readFileSync('out/cross96_' + v + '.tsv', 'utf8').trim().split('\n').map((l) => l.split('\t')).map((r) => [(v === 'Z1' ? 'X' : 'Y') + r[0], ...r.slice(1)])); const R0 = require('fs').readFileSync('/dev/null', 'utf8').trim().split('\n').map((l) => l.split('\t'));
