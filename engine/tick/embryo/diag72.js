// Разбор после чтения ворот шага 72: где живут тишины, неустойчивые в 'LS'. Путь -- как в gate72.js
//: путь строки 0 батареи зародыша как есть; перед каждой тишиной -- радиус 'LC' (как есть) и 'LS' (HOLDS)
...
const w = M.create(seed), LIN = require('./lin.js');
if (C.HOLDS) throw new Error('ворота -- на зародыше как есть');
if (mode === 'нульА') {
...
for (const T of [10, 30]) {
  for (let c = 0; c < 100; c++) {
    ...
    const B = LIN.build(w, C, 'LS', SCH), [b] = LIN.eig([B]);
    if (b.r > 1) {
      const GR = (ch) => (ch <= 2 || ch === 8 ? 'ист' : ch <= 7 ? 'смеси' : ch === 9 ? 'к9' : ch === SCH ? 'S' : 'еда');
      const g = {}; let pz = 0; b.sh.forEach((v, i) => { const k = GR(B.parts[i >> 1].ch) + (i & 1 ? '/сигнал' : '/прогноз'); g[k] = (g[k] || 0) + v; if (i & 1) pz += v; });
      const top = Object.entries(g).sort((x, y) => y[1] - x[1]).slice(0, 4).map(([k, v]) => `${k} ${v.toFixed(2)}`).join(', ');
      const zr = LIN.eig([LIN.build(w, { ...C, SIGNAL: 0 }, 'LS', SCH)])[0].r;
      console.log(`${seed} T=${T} цикл ${c}: LS ${b.r.toFixed(4)}; доля сигналов в ведущем векторе ${pz.toFixed(2)}; ${top}; без путей через сигналы ${zr.toFixed(4)}`);
    }
