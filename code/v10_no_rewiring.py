"""
v0.10: обучение без перестройки контактов (M0) -- изолированное ядро на
базе v08_experience_rewiring.py, БЕЗ изменения зафиксированного v0.8
кода. Единственное содержательное отличие: rewire_policy="none"
обрабатывается как "событие без вмешательства" (расписание получателей
по-прежнему строится и потребляется -- чтобы не сдвинуть RNG-потоки
других частей кода при сравнении -- но do_rewire_event НЕ вызывается,
контакты НЕ меняются). Электрическая динамика (integrate_electrical,
process_fired_batch), STDP-правило и порядок обработки совпадающих
событий -- ПОЛНОСТЬЮ ИДЕНТИЧНЫ v08_experience_rewiring.py (скопированы
без изменений, см. v10_unit_tests.py для явной проверки, что STDP-путь
не затронут добавлением "none"-режима).

Исходный docstring v0.8 (сохранён для контекста истории формул):
---
...
этого:
  ...
  5. Перестройка, назначенная на t (использует граф ДО замены для шага
     4, новый граф действует со следующего пакета) -- ЗДЕСЬ, в v0.10,
     при rewire_policy="none" этот шаг -- НЕ-ОПЕРАЦИЯ: расписание
     потребляется (rewire_event_idx += 1), но contacts/W не меняются.
...
def run_experience_trajectory(
    ...
    for step in range(total_steps):
        ...
        #    next timestamp onward). rewire_policy="none": event marked as
        #    "no-op" but the schedule is STILL consumed (rewire_event_idx
        #    advances) -- so RNG streams are unaffected by which policy is
        #    used, and the schedule stays comparable across M0/M1/M2/M3.
        if completed_steps % rewire_every_steps == 0:
            receiver = (int(schedule[rewire_event_idx])
                        if rewire_event_idx < len(schedule) else None)
            rewire_event_idx += 1
            if rewire_policy == "none":
                event_log.append({"step": completed_steps, "t": completed_steps*DT,
                                   "status": "policy_none_no_op", "receiver": receiver})
