...
for (let r = 1; r <= 5000; r++) {
  ...
  const fw = Sp().map((p) => [p, p.fromWorld]);
  M.round(w); const L = w.L;
  for (const [p, f] of fw) if (w.parts[p.slot] === p) { payS += p.fromWorld - f; nS++; }
...
console.log([cond, seed, med(alive), r2(net).toFixed(3), r2(naive).toFixed(3), r2(kal).toFixed(3), med(bx).toFixed(3),
  gain.toFixed(3), wsS.toFixed(3), wsX.toFixed(3), st.right.toFixed(3), st.nul.toFixed(3), aS.toFixed(3), ageS, (payS / nS).toFixed(3)].join('\t'));
