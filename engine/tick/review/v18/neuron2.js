...
const N      = K('N', WORLD ? 8 * ((DEEP ? 10 : 8) + (EAT ? 1 : 0)) : 64);   // мест (по 8 на канал)
...
const CH = CHW + (EAT ? 1 : 0), V = 1 + SN * SN;
...
function worldStep(w) {
  ...
  if (!WORLD) { for (let k = 0; k < CH; k++) n[k] = 0.9 * c[k] + Math.sqrt(0.19) * g(); }
    ...
    if (DEEP) {                           // источник 8 и канал 9: c9(t+1) = c8(t - DEEP)
      ...
      if (EAT) { n[FCH] = (hid - LOOP * w.abar) / FDIV; w.hid = hid; }
...
function newPart(w, slot, par) {
  ...
  const p = { slot, ch: slot % CH, g, credit: par ? BIRTH / 2 : START,
    ...
    ph: [], ate: 0, nAct: 0, nHit: 0 };   // прошлые намерения (для нуля Б) и счёт действий
...
function create(seed) {
  const w = { rnd: makeRNG(seed), round: 0, parts: [], c: new Float64Array(CH), births: 0,
    ...
    pruned: [0, 0, 0], lost: [0, 0, 0], colon: 0,
    abar: 0, hid: 0, acts: [], fsamp: [], fq: null,    // среднее названное место, выборка F, границы долей
    fs: { acts: 0, hits: 0, rounds: 0, fed: 0, winners: 0, burn: 0, sf: 0, n: 0,
          sum: 0, sum2: 0, actsRelay: 0, hitsRelay: 0, actsBare: 0, hitsBare: 0, spent: 0, paid: 0, byK: new Map() } };  // сколько связей каждого товара отмерло и сколько потеряно со смертью продавца
...
function round(w) {
  ...
  if (EAT) {                              // границы долей, раздача еды, замеры второй половины
    ...
    const truth = binOf(w, f);
    const win = truth >= 0 ? w.acts.filter((a) => a.place === truth) : [];
    if (win.length) { const share = FOOD / win.length;
      for (const a of win) { const q = P[a.slot]; if (q) { q.credit += share; q.ate += share; q.nHit++; } } }
    if (w.round >= ROUNDS / 2) {
      const S = w.fs; S.n++; S.sum += f; S.sum2 += f * f;
      ...
      if (w.fq) { let ok = 0;             // есть ли в этом круге самосогласованное место
        for (let a = 0; a < M_PL; a++) if (binOf(w, (w.hid - LOOP * mPlace(a)) / FDIV) === a) ok++;
        if (ok) S.sf++; }
    }
    w.acts = []; w.abar = 0;
  }
  ...
  for (const p of P) {
    if (!p) continue;
    const pay = (EAT && p.ch === FCH) ? 0 : (shared ? bank / alive : PAY * p.bits);
  ...
  return { alive: A.length, births: w.births, bank: w.deaths.bank, fault: w.deaths.fault,
    ...
    food: EAT ? (() => { const Q = w.fs, F = A.filter((p) => p.ch === FCH);
      ...
      return { share: Q.acts ? Q.hits / Q.acts : NaN,          // ОСНОВНАЯ МЕРА: доля верных действий
        ...
        alive: F.length, gAct: med(F.map((p) => p.g.act)), bitsF: med(F.map((p) => p.bits)) }; })() : null,
